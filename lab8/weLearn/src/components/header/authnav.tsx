...
const Header: React.FC<HeaderProps> = ({type}) => {
    ...
    return (
        <div className='flex justify-between items-center h-18 py-1 px-6 border-b-2 dark:border-dark-card-bg z-[100]'>
            ...
            {type === 'signup' && <a className='absolute right-[100px] text-primary-link-purp' href='/login'>{t('Login')}</a>}
            {type === 'login' && <a className='absolute right-[100px] text-primary-link-purp' href='/signup'>{t('Signup')}</a>}
